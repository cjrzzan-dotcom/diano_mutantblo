...
function getAttackPower(player){
  ...
  return player.baseAtk + player.stats.atk * 2 + eq.atk;
}
...
function buildBattleButtons(player, dungeonKey){
  ...
  return [
    ...
    new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('equipment_view').setLabel('🎒 장비창').setStyle(ButtonStyle.Primary),
...
client.on('messageCreate', async (message)=>{
  ...
  if(command === '!상점'){
    await message.reply({ content:'🏪 상점\n🧪 작은 물약 10G\n🧴 중간 물약 30G\n🧃 큰 물약 100G\n✨ 엘릭서 3000G', components:buildShopButtons() });
    return;
  }
  if(command === '!제작목록'){
...
client.on('interactionCreate', async (interaction)=>{
  ...
  if(id === 'shop'){
    await interaction.deferReply({ ephemeral:true });
    await interaction.editReply({ content:'🏪 상점\n🧪 작은 물약 10G\n🧴 중간 물약 30G\n🧃 큰 물약 100G\n✨ 엘릭서 3000G', components:buildShopButtons() });
    return;
  }
  if(id === 'craft_list'){
    await interaction.deferReply({ ephemeral:true });
